...
"""`;
  ...
  // 출처 = **모델이 실제로 쓴 노하우**(used_sop_ids[0])다. 검색 1위(sops[0])를 그대로 쓰면
  // 답 본문은 A에서 왔는데 '원문 보기'는 B를 여는 오지목이 난다(2026-08-11 P4 실측: 표본 2건 중 1건).
  // 못 찾으면(빈 배열·미지의 id) 검색 1위로 물러난다 — 출처 없는 답을 만들지는 않는다.
  const primary = sops.find((s: any) => s.id === out.used_sop_ids?.[0]) ?? sops[0];
...
`
  ...
  · ★situation 은 **질문이 나오는 상황이 아니라 [원문](= 사장님의 답)의 내용**이다.
    "앞치마가 필요한 경우"(✗ 답이 사라짐) / "앞치마 위치: 포스기 아래 서랍 두 번째 칸"(○)\n`
    : '';
...
- ★값 보존(가장 흔한 유실): [원문]에 있는 **구체적인 값**(위치·번호·비밀번호·연락처·시각·이름·수량)은 반드시 situation 에 그대로 옮겨라.
  · 예: situation="앞치마 위치: 포스기 아래 서랍 두 번째 칸" / situation="와이파이 비밀번호: cafe2026!"
  · 값을 빼고 "○○ 확인"·"○○가 필요한 경우" 같은 껍데기만 남기지 마라 — 그 카드는 물어본 사람에게 아무것도 알려주지 않는다.
  · 비밀번호·연락처도 **매장 내부 기록**이므로 값을 그대로 적는다(공유 주의는 dont 에 적어라).
