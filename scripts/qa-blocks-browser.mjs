...
async function main() {
  ...
  try {
    ...
    await po.waitForTimeout(2500);
    // ★2026-08-26 B안: 주인공이 **퀴즈에서 노하우로** 바뀌었다 — 커버리지 히어로 + 그 아래 Primary.
    //   여기 단언 셋은 08-07 구조('거르기 3칸'·'업무 진도 행'·'2층 링크 한 줄')를 재고 있었고,
    //   08-11 재설계가 그 셋을 전부 없앴는데 **따라오지 않아 계속 red 였다**(제품과 무관한 실패).
    //   같은 일이 또 나지 않게 **데이터에 안 기대는 것**만 잰다 — 시드가 무엇이든 참인 것들이다.
    check('B3-0 퀴즈 화면 진입 — 로딩이 끝났다', !(await see(po, '퀴즈를 불러오는 중')));
    // 히어로 문구는 재료 유무로 셋 중 하나다. 퀴즈가 0건이면 빈 화면 문구가 그 자리에 선다.
    const heroLine =
      (await see(po, '직원이 아는지 아직 안 물어봤어요')) ||
      (await see(po, '전부 문제로 냈어요')) ||
      (await see(po, '문제를 낼 노하우가 없어요')) ||
      (await see(po, '먼저 노하우가 필요해요')) ||
      (await see(po, '아직 만든 퀴즈가 없어요'));
    check('B3-1 히어로가 주인공이다 — 커버리지 문장 또는 빈 화면 문구', heroLine);
    // 만들기는 헤더에서 내려왔다(그 자리는 보관함이다) — 헤더로 되돌아가면 Primary 가 둘이 된다.
    check('B3-2 만들기는 헤더가 아니라 본문 Primary 다', !(await see(po, '＋ 만들기')));
