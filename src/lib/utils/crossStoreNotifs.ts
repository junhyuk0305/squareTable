...
export function storeUnreadCount(d: UnitNotifData, role: string, me: string, today: string): number {
  return role === 'owner'
    ? ownerUnreadCount(d.queue, d.suggestions, d.swaps, d.pending, d.feed, me)
    : juniorUnreadCount(d.feed, d.swaps, me, today, d.taskTemplates, d.done);
}

/** 매장 하나의 알림 목록 — 기존 빌더 재사용(교대 시간표기용 ShiftTemplate 은 cross-store 미제공 → 날짜만). */
export function buildStoreNotifs(d: UnitNotifData, role: string, me: string, today: string): CrossNotifRow[] {
  const nameOf = (id: string) => (id === me ? '나' : d.names[id] || (role === 'owner' ? '직원' : '동료'));
  const rows: (JuniorNotif | OwnerNotif)[] =
    role === 'owner'
      ? buildOwnerNotifications({ queue: d.queue, suggestions: d.suggestions, swaps: d.swaps, pending: d.pending, nameOf, feed: d.feed, userId: me })
      : buildJuniorNotifications({ feed: d.feed, swaps: d.swaps, templates: [], nameOf, userId: me, today, taskTemplates: d.taskTemplates, done: d.done });
