...
const LEAK_RE =
  // ★ '(알바가|직원이) 바로 따라 할 수 있게'는 EXTRACTION_MASTER 첫 줄의 문구다(extraction-master.ts).
  //   워딩 기준 §3.3으로 프롬프트가 '직원이'로 바뀌었지만, 옛 문구가 섞인 응답도 계속 잡아야 하므로 둘 다 둔다.
  //   프롬프트 문구를 또 바꾸면 여기도 같이 바꾼다(안 바꾸면 누출 탐지가 조용히 죽는다).
  /매장 노하우.*정리|원문.*(분석|정리)|(알바가|직원이) 바로 따라 할 수 있게|entries\s*배열|scale_prompt|situation\b|category를?\s*분류|꼬리질문\(followups\)|정리할.*원문.*입력/i;
