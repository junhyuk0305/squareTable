 ...
 *
 * ════════════════════════════════════════════════════════════════════════
 * ★★ 2026-08-27 실측 — **t5 형태 3종은 자동 출제에 사실상 안 나온다. 그대로 둔다.**
 *
 *   `qa:quiz-pick` 으로 store_001 노하우 40건을 재 보면 case_pick·quick_judge·branch_path 가
 *   **한 번도 안 뽑힌다.** 재료가 없어서가 아니다 — 셋 다 생성은 잘 된다(2/2 · 근거 68~87%).
 *   이유는 순위다: detectKinds 가 t5 를 40건 중 **3건**에서만 잡고, 그 3건마저 **전부 4순위**다
 *   (앞이 t3>t2>t1). 호출부가 max:1~3 이라 t5 까지 못 간다.
 *
 *   ★고치지 않기로 했다(사용자 확정). t5 가 늘 지는 상대가 **t3(금지)** 이기 때문이다 —
 *   dont 가 적혀 있다는 건 그 매장에서 **이미 사고가 났다**는 뜻이라 07-29 가 출제 1순위로
 *   못박은 것이다. 금지가 적힌 노하우에서 금지 대신 갈래를 묻는 건 교육적으로 후퇴다.
 *   t5 를 살리는 옳은 길은 순위 조작이 아니라 **재료가 늘어나는 것**(사장이 situation 에 조건을
 *   적는 것)이고, 사장이 형태를 직접 고르는 경로(planFor)로는 지금도 나온다.
 *   → 여기 점수·순서를 t5 때문에 건드리지 말 것. 같은 조사를 또 하지 않도록 결론을 남긴다.
 *
 * ⚠️ 함께 드러난 것(별개 축 · 미수정) — **같은 노하우는 영원히 같은 문항이 된다.**
 *   rotationSeed 가 노하우 id 해시라 회전축이 고정이다. 위 07-29 주석("같은 노하우가 세 번째
 *   나올 때 형태가 달라야 복습이 견딘다")은 **자동 경로에서 지켜지지 않는다** — quiz-new 는
 *   opts.existing 을 넘기지 않아 몇 번째 출제인지 모른다(델타를 쓰는 곳은 QuizEditorSheet 뿐).
 *   고치려면 호출부까지 같이 손대야 해서 이번 스코프 밖으로 뒀다.
 * ════════════════════════════════════════════════════════════════════════
 */
