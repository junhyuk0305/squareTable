...
export function TodoScreen({
  ...
  // 내가 볼 수 있는 할일: 공유 or 내 개인 (사장은 개인도 모두).
  const visible = useMemo(
    () => templates.filter((t) => (t.scope ?? 'shared') !== 'private' || t.ownerId === me || isOwner),
    [templates, me, isOwner],
  );
              ...
              {!isCol && (
                <View style={s.groupList}>
                  {g.tasks.map((t, i) => {
                    ...
                    return (
                      <View key={t.id} style={[s.item, i === g.tasks.length - 1 && { borderBottomWidth: 0 }]}>
                        ...
                        {assignedName ? <Text style={s.assignTag}>담당 {assignedName}</Text> : isMine ? <Text style={s.mineTag}>나만</Text> : null}
                        {onAttachPhoto && !on && (
...
const s = StyleSheet.create({
  ...
  todayBtn: { marginLeft: 'auto', borderWidth: 1, borderColor: InkColors.line, backgroundColor: InkColors.bg, borderRadius: 99, paddingHorizontal: 9, paddingVertical: 3 },
  ...
  cell: { width: `${100 / 7}%`, minHeight: 42, alignItems: 'center', justifyContent: 'center', gap: 3, borderRadius: 11, paddingVertical: 3 },
  ...
  legendInline: { flexDirection: 'row', gap: 4, marginLeft: 'auto' },
  ...
  item: { flexDirection: 'row', alignItems: 'center', gap: 10, paddingVertical: 9, paddingHorizontal: 4, borderBottomWidth: 1, borderBottomColor: InkColors.paper },
  scopeBar: { width: 4, height: 22, borderRadius: 99 },
