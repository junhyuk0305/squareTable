...
export function OwnerKnowhowBrowse({ onSelect }: { onSelect: (id: string) => void }) {
  ...
  const listFiltered = useMemo(() => {
    let list = onlyNeedsReview ? baseFiltered.filter(needsVerify) : baseFiltered;
    if (sort !== 'category') {
      list = [...list].sort((a, b) => {
      ...
      {!hasEntries ? (
        ...
        <>
          ...
          <View style={styles.viewToggle}>
            <Pressable onPress={() => setView('dashboard')} style={[styles.viewToggleBtn, view === 'dashboard' && styles.viewToggleBtnOn]}>
          ...
          {view === 'list' && (
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.sortRow}>
              {onlyNeedsReview && (
                <Pressable onPress={() => setOnlyNeedsReview(false)} style={[styles.sortChip, styles.sortChipActive]}>
                  <Text style={styles.sortChipActiveText}>미검증만 ✕</Text>
                </Pressable>
              )}
              {SORTS.map((s) => {
                const on = sort === s.key;
...
const styles = StyleSheet.create({
  ...
  sortChipActive: { backgroundColor: BrandColors.gold },
  sortChipActiveText: { fontSize: 12, fontWeight: '800', color: InkColors.ink },
