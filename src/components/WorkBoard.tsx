...
import { useWorkStore, type NewTask } from '@/lib/store/useWorkStore';
...
export function WorkBoard({ role }: { role: 'owner' | 'junior' }) {
  ...
  const stream = useMemo(
    () => feed.filter((f) => f.kind === 'message' || f.kind === 'task_done').sort((a, b) => a.createdAt.localeCompare(b.createdAt)),
    [feed],
  );
  ...
  const headerOptions =
    view === 'chat'
      ...
      : {
          ...
          headerLeft: () => (
            <Pressable onPress={() => setView('chat')} hitSlop={8} style={({ pressed }) => [{ paddingHorizontal: 8 }, pressed && { opacity: 0.6 }]}>
    ...
    <SafeAreaView style={st.safe} edges={['bottom']}>
      ...
      {composer.open && (
        <TaskComposerModal
          ...
          onSubmit={(input: NewTask) => addTask(input)}
          isOwner={isOwner}
          me={userId}
          today={today}
...
const st = StyleSheet.create({
  ...
  nav: { flexDirection: 'row', gap: 6, paddingRight: 6 },
  navBtn: { flexDirection: 'row', alignItems: 'center', gap: 4, height: 32, paddingHorizontal: 10, borderRadius: Radius.sm, borderWidth: 1, borderColor: InkColors.line, backgroundColor: InkColors.bg },
  navText: { fontSize: 12, fontWeight: '700', color: InkColors.ink },
